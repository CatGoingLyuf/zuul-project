...
public abstract class AbstractRibbonCommand<LBC extends AbstractLoadBalancerAwareClient<RQ, RS>, RQ extends ClientRequest, RS extends HttpResponse>
	...
	protected ClientHttpResponse run() throws Exception {
		...
		if (retryableClient) {
			...
			// 默认retryableClient是false,会走到下面的executeWithLoadBalancer
			// 进入executeWithLoadBalancer 调用 command.submit 方法
			// 下一步在selectServer()中 获取loadBalancerContext.getServerFromLoadBalancer();
			// 在getServerFromLoadBalancer中获得一个ILoadBalancer,会调用ILoadBalancer#chooseServer方法来选择最终的服务
